...
| :--- | :--- | :--- |
| **引用文献/数据来源 (Cite Sources)** | **+40%** | 酌情自然引用精神病学、认知行为疗法（CBT）的大牛和文献（比如 Dr. Russell Barkley ）。**（注：本站非必要不强求堆砌，以可读性优先）** |
...
为了保持神经多样性受众阅读时的清爽与专注体验，**非必要不强行引用文献**。

*格式规范*：
*   **自然合理植入**：仅在阐述极其核心的医学病理或核心理论机制时，可酌情自然融入指向权威文献数据库的超链接。严禁为了凑数或盲目追求 GEO 引用提升率而生硬、防御性地堆砌学术出站链接。
*   **可选文献章节**：如果文章中未进行学术引用，则**无需在文末添加** `## 引用来源`（英文版 `## References`）模块。若有确实必要的引用，再按如下格式添加：
  ```markdown
  ## 引用来源
