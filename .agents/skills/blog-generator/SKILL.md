...
### B. MDX 标点与混排加粗 Bug 规避
- 在 MDX 中，当中文字符、中文引号（如 `「`）、括号（如 `（ASD）`）与 Markdown 加粗符号 `**` 直接连写粘连时，编译器常会因语法边界歧义而将其解析为字面量 `**` 暴露在页面上。另外，如果因为渲染不成功而在星号内外增加空格，又会导致页面产生难看的多余间距，破坏中文排版美感。
- **规避方案**：在中文字符、中文标点或中英括号混排需要加粗时，**一律改用标准 HTML 的 `<strong>` 标签包裹**。例如：
  ```html
  换成<strong>「我可以……」</strong>：
  作为拥有<strong>自闭特质（ASD）</strong>的成人
  ```
  *这能彻底规避 Markdown 加粗对空格的特殊要求，确保在 Astro/MDX 页面上 100% 完美无瑕地渲染，且没有多余的空白占位。*
...
- **去英文标注**：中文正文阐述专业概念与神经多样性特质时，只要已具有明确通用的中文名称，**一律禁止**在其后方以括号形式标注英文简称（如禁止：`高敏感 (HSP)`、`自闭特质 (ASD)`、`多动特质 (ADHD)`、`核磁共振 (fMRI)`，应直接书写为 `高敏感`、`自闭特质`、`多动特质`、`核磁共振`）。这有助于减少对读者的认知干扰和页面视觉噪音。医疗/学术界的通用专有简称（如 RSD, CBT）可以酌情单独保留，但不需要中英双重标注。
- **严禁混入英文连/助词 (No Pseudo-English Connectives)**：在编写或翻译中文正文时，**绝对禁止**直接混入英文的连词、助词或介词（例如用 `and` 代替 `和/与`，用 `or` 代替 `或`，用 `of` 代替 `的`，用 `but` 代替 `但`）。这是由于翻译工具不当遗留的典型 AI 翻译腔，在最终发布前必须仔细排查并完全替换为自然的中文句式（例如禁写：“社交潜规则 and 含糊指令”、“死寂 of 房间”）。
...
1. **Frontmatter 配置与 GEO 标题优化**：
   ...
   - **双语标题 GEO 命名规范 (非强制镜像对照)**：中英文标题的内容与命名思路**无需镜像一致**。中文更注重情绪共鸣与场景代入，英文注重痛点召回与清爽解答。应针对各自语境独立优化，使其兼具“人声温度”与“AI语义召回力”。
   - **GEO 标题设计多元化法则 (告别死板套路)**：
     避免陷入“数字 + 如何 + 权威实证”的千篇一律 AI 八股文格式。好的标题应当像真实的人在说话，用多样化的句式和视角去吸引人类读者与 AI 引擎的注意：
     * **场景共鸣与痛点直击型 (Situation & Prompt Match)**：
       * *核心*：直接描绘神经多样性受众最具体的“死机”画面。这不仅是极佳的同理心切入，也是对用户向 AI 进行长句描述性提问（如“下午有事一整天就废了怎么办”）的精准语义匹配。
       * *示例*：`下午三点开会，一整天就废了？` / `为什么你明知道有急事却卡在沙发上动弹不得？`
     * **反差冲突与观念颠覆型 (Antithesis & Insight)**：
       * *核心*：打破主流“自律神话”和优绩主义规训。通过制造常识冲突，提供本站独创的“反效率”解法，极易被 AI 引擎提取为“独特或差异化观点”推荐给用户。
       * *示例*：`自律越狠，死机越快：为什么意志力管理是对大脑的霸凌` / `放弃极致效率，做一个及格家`
     * **同理心接纳与温柔放过型 (Gentle Validation)**：
       * *核心*：不居高临下，不制造新的焦虑。以温和自嘲的同伴视角，提供“安全感”和底线生存智慧。
       * *示例*：`告别“我应该”，拥抱“我可以”` / `允许自己有“零电量”的一天`
     * **轻量方案与结构化暗示型 (Low-friction Signposts - 酌情使用)**：
       * *核心*：暗示文章包含低门槛、无痛可操的方法，便于 AI 引擎将其作为步骤清单直接召回，但要避免使用生硬的“科学实证/干货满满”等虚伪的 AI 词汇。
       * *示例*：`写给 ADHD 的 3 个“微步破冰”物理自救技巧` / `适合 ADHD 大脑的“多巴胺工作流”替代方案`
     * **核心实体词的自然融入 (Invisible SEO)**：
       * 在确保上述四种人声温度句式的前提下，将核心实体词（如 `ADHD 任务瘫痪`、`RSD`、`Time Blindness`）自然融入标题或描述中，以此维持在 AI 知识图谱中的检索关联度，而不是生硬堆凑。
...
7. **文献引用 (References - 可选)**：
   - **非强求性原则**：为了给神经多样性受众最清爽的阅读体验，非必要不强行引用文献，切忌生硬堆砌。如果文章中未进行学术引用，则**无需在文末添加该模块**。
   - 若有确实需要的核心引用，中文版统一使用 `## 引用来源` 作为二级标题，英文版统一使用 `## References` 作为二级标题。
