...
export async function GET() {
  ...
  txt += `## Core Pages\n`;
  txt += `- [Home](https://adhdos.app/blog) - Chinese main page\n`;
  txt += `- [English Home](https://adhdos.app/blog/en) - English main page\n`;
  txt += `- [Agent Guide](https://adhdos.app/blog/agent) - AI/Agent integration documentation\n\n`;
  
  txt += `## Chinese Articles (zh)\n`;
  for (const post of zhPosts) {
    const slug = getSlug(post.id);
    txt += `- [${post.data.title}](https://adhdos.app/blog/${slug}) - ${post.data.description}\n`;
  }
  
  txt += `\n## English Articles (en)\n`;
  for (const post of enPosts) {
    const slug = getSlug(post.id);
    txt += `- [${post.data.title}](https://adhdos.app/blog/en/${slug}) - ${post.data.description}\n`;
  }
